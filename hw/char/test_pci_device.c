...
#include "../../include/char/test_pci.h"
...
void
test_do_something (TestPCIState * s)
{
...
void
test_show_cdmabuf (TestPCIState * s)
{
...
void
test_show_sdmabuf (TestPCIState * s)
{
  PCIDevice *pci_dev = PCI_DEVICE (s);
  int i;
...
test_pci_mmio_read (void *opaque, hwaddr addr, unsigned size)
{
  ...
  if ((TEST_MEM && addr >= TEST_MEM && addr < TEST_MEM + TEST_MMIO_DATASIZE)
      || (!TEST_MEM && addr < TEST_MEM + TEST_MMIO_DATASIZE))
    {
...
static void
test_pci_mmio_write (void *opaque, hwaddr addr, uint64_t val, unsigned size)
{
  TestPCIState *s = opaque;
...
static void
test_pci_pio_write (void *opaque, hwaddr addr, uint64_t val, unsigned size)
{
  TestPCIState *s = opaque;
  PCIDevice *pdev = PCI_DEVICE (s);
  ...
  if ((TEST_PIO && addr >= TEST_PIO && addr < TEST_PIO_DATASIZE) ||
      (!TEST_PIO && addr < TEST_PIO_DATASIZE))
    {
...
static void
qdev_test_pci_reset (DeviceState * ds)
{
...
static void
test_pci_class_init (ObjectClass * klass, void *data)
{
